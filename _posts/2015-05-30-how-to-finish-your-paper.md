...
<p style="text-color:read">更新公式居中，标号右对齐，在最后面其它</p>**其它＃3**
...
3.公式居中，标号居右。
- 先工具栏－视图－选中**标尺**,然后编辑页面上方就出现了标尺。
- 插入在你需要插入公式的地方，插入公式
- 让公式居中(ctrl + E)，然后鼠标放到标尺栏上面，左键hold on，会出现一个对准线标记，将其对准在公式的最左端。
- 再在标尺栏上面标记公式标号的左端位置，根据公式标号5号字体，大概离页面3左右的距离（具体位置，根据你的情况而定，然后再调整）
- 让公式居左，在公式右边紧接着写公式标号，然后在公式左端按一下`tab`键，再在标号前面按一下`tab`键，就分别让公式和标号跳到你最开始标记的位置了。

![pic15](http://7xj6ej.com1.z0.glb.clouddn.com/word15.jpg)
